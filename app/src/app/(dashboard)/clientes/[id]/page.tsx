...
export default async function ClientDetailPage({ params }: { params: { id: string } }) {
  ...
  const { data: client, error } = await supabase
    .from('clients')
    .select('*, equipment(id, model, serial_number, status), services(id, work_order, status)')
    .eq('id', params.id)
    .maybeSingle()
  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-col gap-4 rounded-2xl border border-brand-100 bg-brand-50/60 p-5 md:flex-row md:items-center md:justify-between">
        <div>
          <p className="text-xs uppercase tracking-[0.22em] text-brand-700/70">Cliente</p>
          <h1 className="mt-2 text-2xl font-semibold text-brand-900">{client.name}</h1>
          <p className="mt-1 text-sm text-brand-900/70">{client.legal_name ?? 'Razão social não informada'}</p>
        </div>

        {canDelete && <DeleteRecordButton endpoint={`/api/clientes/${params.id}`} redirectUrl="/clientes" />}
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <div className="rounded-2xl border border-brand-100 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <InfoBlock label="CNPJ" value={client.cnpj ?? '—'} />
            <InfoBlock label="Contato principal" value={client.primary_contact ?? '—'} />
            <InfoBlock label="E-mail" value={client.email ?? '—'} />
            <InfoBlock label="Telefone" value={client.phone ?? '—'} />
            <InfoBlock label="Status" value={client.status === 'active' ? 'Ativo' : 'Inativo'} />
            <InfoBlock label="Última atualização" value={formatDateTime(client.updated_at)} />
          </div>

          <div className="mt-6">
            <h3 className="text-sm font-semibold uppercase tracking-wide text-brand-700/70">Notas</h3>
            <p className="mt-2 whitespace-pre-wrap text-sm text-brand-900/80">{client.notes ?? 'Nenhuma nota registrada.'}</p>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-brand-100 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 space-y-3 text-sm">
            <div className="rounded-lg bg-brand-50 p-3"><p className="text-brand-700/70">Equipamentos</p><p className="mt-1 text-brand-900">{client.equipment?.length ?? 0} vínculo(s)</p></div>
            <div className="rounded-lg bg-brand-50 p-3"><p className="text-brand-700/70">Serviços</p><p className="mt-1 text-brand-900">{client.services?.length ?? 0} ordem(es)</p></div>
      ...
      <div className="rounded-2xl border border-brand-100 bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 space-y-3">
          {(audit ?? []).length ? (
            ...
            <p className="text-sm text-brand-900/60">Nenhum evento de auditoria para este cliente.</p>
          )}
        </div>
      </div>
    </section>
