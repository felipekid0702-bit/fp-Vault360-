...
export default async function NewInspectionPage({
  ...
  if (!searchParams.equipmentId || !searchParams.templateId) {
    ...
    return (
      <section><h1 className="text-xl font-semibold">Nova Inspeção</h1><p className="mt-1 text-sm text-brand-900/70">Selecione o equipamento e o checklist que serão usados.</p><div className="mt-6 overflow-hidden rounded-lg border border-brand-100 bg-white"><table className="w-full text-sm"><thead className="bg-brand-50 text-left text-xs uppercase text-brand-700/70"><tr><th className="px-4 py-3">Equipamento</th><th className="px-4 py-3">Proprietário / Cliente</th><th className="px-4 py-3">Checklist disponível</th><th className="px-4 py-3">Ação</th></tr></thead><tbody className="divide-y divide-brand-50">{targets.equipment.flatMap((item: any) => targets.templates.filter((template: any) => !template.category_id || template.category_id === item.category_id).map((template: any) => <tr key={`${item.id}-${template.id}`}><td className="px-4 py-3 font-medium">{item.category?.name ?? '—'} · {item.model} {item.manufacturer?.name ? `· ${item.manufacturer.name}` : ''} {item.serial_number ? `· ${item.serial_number}` : ''}</td><td className="px-4 py-3">{item.owner_type === 'client' ? item.client?.name : 'FP Soluções'}</td><td className="px-4 py-3">{template.name}</td><td className="px-4 py-3"><Link href={`/inspecoes/nova?equipmentId=${item.id}&templateId=${template.id}`} className="text-brand-700 underline">Iniciar</Link></td></tr>))}{targets.templates.map((template: any) => <tr key={`new-${template.id}`}><td className="px-4 py-3 text-brand-700/70">Novo equipamento</td><td className="px-4 py-3">—</td><td className="px-4 py-3">{template.name}</td><td className="px-4 py-3"><InlineEquipmentCreate templateId={template.id} categories={categories} /></td></tr>)}{(!targets.equipment.length || !targets.templates.length) && <tr><td colSpan={4} className="px-4 py-10 text-center text-brand-700/60">Cadastre equipamentos e checklists ativos para iniciar uma inspeção.</td></tr>}</tbody></table></div></section>
    )
  }
