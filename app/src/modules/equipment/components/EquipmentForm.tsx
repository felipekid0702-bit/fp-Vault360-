...
export function EquipmentForm({
  ...
  return <>
    ...
    {open && <form onSubmit={submit} className="mt-6 rounded-lg border border-brand-100 bg-white p-5"><div className="grid gap-4 md:grid-cols-3">
      ...
      {ownerType === 'client' && <label className="text-sm font-medium">Serviço<select required value={serviceId} onChange={(event) => setServiceId(event.target.value)} className="mt-1 block w-full rounded-md border border-brand-100 px-3 py-2 font-normal"><option value="">Selecione um serviço</option>{services.filter((service) => !service.client_id || service.client_id === clientId).map((option) => <option key={option.id} value={option.id}>{option.name ?? option.work_order}</option>)}</select></label>}
