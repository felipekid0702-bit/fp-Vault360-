...
export function InspectionEditForm({ inspection }: { inspection: { id: string; notes?: string | null; inspection_location?: string | null; next_due_date?: string | null; result?: string | null; verdict?: string | null; items?: EditItem[] } }) {
  ...
  return <div>
    ...
    {open && <form onSubmit={submit} className="mt-4 space-y-4 rounded-lg border border-brand-100 bg-brand-50 p-4">
      ...
      <label className="text-sm font-medium">Próximo vencimento (dd/mm/aaaa)<input required pattern="[0-9]{2}/[0-9]{2}/[0-9]{4}" inputMode="numeric" value={nextDueDate} onChange={(event) => setNextDueDate(event.target.value)} placeholder="31/12/2026" className="mt-1 block w-full rounded border bg-white px-3 py-2 font-normal" /></label>
