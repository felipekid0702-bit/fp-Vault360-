  ...
  return (
    <div className="flex min-h-screen">
      <aside className="w-60 shrink-0 border-r border-brand-100 bg-white px-4 py-6">
        <Image src="/brand/fp-vault360-logo.svg" alt="FP Vault360°" width={140} height={50} />
        <nav className="mt-8 space-y-1">
          {NAV_ITEMS.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="block rounded-md px-3 py-2 text-sm text-brand-900/80 transition hover:bg-brand-50 hover:text-brand-700"
            >
              {item.label}
            </Link>
          ))}
