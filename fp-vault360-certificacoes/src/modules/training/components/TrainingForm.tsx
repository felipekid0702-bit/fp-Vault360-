...
export function TrainingForm({ certifications = [] }: { certifications?: CertificationOption[] }) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch('/api/treinamentos', {
        ...
        body: JSON.stringify({ name, provider: provider || undefined, certification_id: certificationId || undefined }),
      })
      ...
      {isOpen && (
        <form onSubmit={handleSubmit} className="mt-6 rounded-lg border border-brand-100 bg-white p-5">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <label className="text-sm font-medium">Certificação vinculada (opcional)
              <select value={certificationId} onChange={(event) => setCertificationId(event.target.value)} className="mt-1 block w-full rounded-md border border-brand-100 px-3 py-2 font-normal">
