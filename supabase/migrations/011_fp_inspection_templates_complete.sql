...
begin
  ...
  select id into v_template_id from checklist_templates where template_code = 'FP01' and tenant_id is null;
  insert into checklist_items (template_id, section, label, description, order_index, is_critical, evidence_required)
  select v_template_id, v.section, v.label, v.description, v.order_index, v.is_critical, v.evidence_required
  from (values
  ...
  where v_template_id is not null
    and not exists (select 1 from checklist_items i where i.template_id = v_template_id and i.label = v.label);

  select id into v_template_id from checklist_templates where template_code = 'FP02' and tenant_id is null;
  insert into checklist_items (template_id, section, label, description, order_index, is_critical, evidence_required)
  select v_template_id, v.section, v.label, v.description, v.order_index, v.is_critical, v.evidence_required
  from (values
  ...
  select id into v_template_id from checklist_templates where template_code = 'FP03' and tenant_id is null;
  insert into checklist_items (template_id, section, label, description, order_index, is_critical, evidence_required)
  select v_template_id, v.section, v.label, v.description, v.order_index, v.is_critical, v.evidence_required
  from (values
  ...
  select id into v_template_id from checklist_templates where template_code = 'FP04' and tenant_id is null;
  insert into checklist_items (template_id, section, label, description, order_index, is_critical, evidence_required)
  select v_template_id, v.section, v.label, v.description, v.order_index, v.is_critical, v.evidence_required
  from (values
  ...
  select id into v_template_id from checklist_templates where template_code = 'FP05' and tenant_id is null;
  insert into checklist_items (template_id, section, label, description, order_index, is_critical, evidence_required)
  select v_template_id, v.section, v.label, v.description, v.order_index, v.is_critical, v.evidence_required
  from (values
  ...
  select id into v_template_id from checklist_templates where template_code = 'FP06' and tenant_id is null;
  insert into checklist_items (template_id, section, label, description, order_index, is_critical, evidence_required)
  select v_template_id, v.section, v.label, v.description, v.order_index, v.is_critical, v.evidence_required
  from (values
  ...
  select id into v_template_id from checklist_templates where template_code = 'FP07' and tenant_id is null;
  insert into checklist_items (template_id, section, label, description, order_index, is_critical, evidence_required)
  select v_template_id, v.section, v.label, v.description, v.order_index, v.is_critical, v.evidence_required
  from (values
  ...
  select id into v_template_id from checklist_templates where template_code = 'FP08' and tenant_id is null;
  insert into checklist_items (template_id, section, label, description, order_index, is_critical, evidence_required)
  select v_template_id, v.section, v.label, v.description, v.order_index, v.is_critical, v.evidence_required
  from (values
  ...
  select id into v_template_id from checklist_templates where template_code = 'FP09' and tenant_id is null;
  insert into checklist_items (template_id, section, label, description, order_index, is_critical, evidence_required)
  select v_template_id, v.section, v.label, v.description, v.order_index, v.is_critical, v.evidence_required
  from (values
  ...
  select id into v_template_id from checklist_templates where template_code = 'FP10' and tenant_id is null;
  insert into checklist_items (template_id, section, label, description, order_index, is_critical, evidence_required)
  select v_template_id, v.section, v.label, v.description, v.order_index, v.is_critical, v.evidence_required
  from (values
  ...
  select id into v_template_id from checklist_templates where template_code = 'FP11' and tenant_id is null;
  insert into checklist_items (template_id, section, label, description, order_index, is_critical, evidence_required)
  select v_template_id, v.section, v.label, v.description, v.order_index, v.is_critical, v.evidence_required
  from (values
  ...
  select id into v_template_id from checklist_templates where template_code = 'FP12' and tenant_id is null;
  insert into checklist_items (template_id, section, label, description, order_index, is_critical, evidence_required)
  select v_template_id, v.section, v.label, v.description, v.order_index, v.is_critical, v.evidence_required
  from (values
